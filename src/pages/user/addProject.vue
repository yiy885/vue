  ...
  <el-container class="project-container">
    <el-main>
      ...
      >
          ...
          <template v-else-if="activeStep === 1">
            <el-form :model="formValues" :rules="rules" ref="formRef" label-width="120px">
              ...
              <el-form-item label="專案類別">
                <el-select v-model="category.value.value" placeholder="請選擇類別">
                  <el-option
              ...
              <el-form-item
                label="專案預視圖"
                prop="previewImage"
                :error="previewImage.errorMessage.value"
              >
                <el-upload
                  ref="uploadRef"
        ...
        <template #footer>
          <el-button @click="prevStep" v-if="activeStep > 0" :disabled="isSubmitting"
            >返回上一步</el-button
          >
          <el-button v-if="activeStep === 1" @click="goHome">沒有要新增，回首頁</el-button>
          <el-button v-if="activeStep < 2" @click="nextStep" :disabled="isSubmitting"
            >下一步</el-button
          >
...
const rules = {
  ...
  previewImage: [
    { required: true, message: '請上傳專案預視圖，盡量為正方形比例', trigger: 'change' },
  ],
}
...
const {
  ...
  validationSchema: yup.object({
    ...
    category: yup.string().required('分類是必填的').oneOf(categoryValues, '請選擇有效的專案類別'),

    createdBy: yup.string().required('創建者是必填的'),

    previewImage: yup.string().required('請上傳專案預視圖，盡量為正方形比例'),
  }),
