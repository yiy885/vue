        ...
        >
          <template #item="{ element: module, index }">
            <el-card class="module-card" shadow="hover" :class="{ 'error': hasModuleError(index) }">
              ...
              <el-form :model="module" :rules="formRules(module)" :ref="(el) => moduleFormRefs[index] = el">
                <div class="module-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                    ...
                    <el-button type="danger" :icon="Delete" circle @click="removeModule(index)" size="small"></el-button>
                </div>
                ...
                <template v-else-if="module.type === 'image'">
                  ...
                  <el-form-item prop="content.alt.value">
                    <el-input
                      v-model="module.content.alt.value"
                      placeholder="圖片描述"
                    />
                  </el-form-item>
                </template>
                ...
                <template v-else-if="module.type === 'custom'">
                  <el-form-item prop="content.customField.value">
                    <el-input
                      v-model="module.content.customField.value"
                      placeholder="自訂內容"
                    />
                  </el-form-item>
                </template>
      ...
      if (rule === 'required') {
        ...
        fieldRules.push({
          validator: (r, val, callback) => {
            if ((typeof val === 'string' && val.length > max) || (typeof val === 'number' && val > max)) {
        ...
        fieldRules.push({
          validator: (r, val, callback) => {
            if ((typeof val === 'string' && val.length < min) || (typeof val === 'number' && val < min)) {
...
const loadPage = async () => {
  loading.value = true;
  try {
    const response = await pageService.getPageByUserId(userStore.id);
...
.el-input, .el-select, .el-color-picker {
